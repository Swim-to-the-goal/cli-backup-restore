...
#Execution of the restore command based on the type of database
    try:
        ...
        if db_config['type'] == 'postgresql':
            command = f"PGPASSWORD={db_config['password']} psql -h {db_config['host']} -p {db_config['port']} -U {db_config['user']} -d {db_config['database']} -f {download_path}"
        elif db_config['type'] == 'mysql':
            command = f"mysql -h {db_config['host']} -P {db_config['port']} -u {db_config['user']} -p{db_config['password']} {db_config['database']} < {download_path}"
